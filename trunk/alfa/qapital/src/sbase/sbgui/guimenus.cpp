...
*/
GUIMenus::GUIMenus(QWidget *padre, const char *nombre, int filas, int columnas, int mnid)
...
GUIMenus::GUIMenus(QWidget *padre, const char *nombre, GUILectorXml *handler, int mnid ) : QButtonGroup(padre, nombre), mnid(mnid)
{
...
void GUIMenus::generarMenus(int n, int m)
{
	int gmnContador = 0;
	for(int i = 0 ; i < n; i++)
	{
...
*/
void GUIMenus::gmnCambiarTextoDeBoton(QString nuevotexto, int id)
{
...
int GUIMenus::gmnNumeroDeBotones()
{
	return losBotones.count();
}

int GUIMenus::gmnId() const
{
	return mnid;
}
